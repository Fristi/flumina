...
object ReplicaAssignment {
  val codec: Codec[ReplicaAssignment] =
    (("partitionId" | int32) :: ("replicas" | kafkaArray(int32))).as[ReplicaAssignment]
}
...
object TopicResponse {
  val codec: Codec[TopicResponse] =
    (("topic" | kafkaRequiredString) :: ("kafkaResult" | KafkaResult.codec)).as[TopicResponse]
}
