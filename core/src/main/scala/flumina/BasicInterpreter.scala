...
final class BasicInterpreter[F[_]](requestResponse: KafkaBrokerRequest => F[BitVector])(implicit F: MonadError[F, Throwable]) extends (KafkaA ~> ReaderT[F, KafkaContext, ?]) {
  ...
  private def syncGroup(groupId: String, generationId: Int, memberId: String, assignments: Seq[GroupAssignment]) = {
    ...
    for {
      assignmentRequest <- ReaderT[F, KafkaContext, List[SyncGroupGroupAssignmentRequest]](_ => makeAssignmentRequest())
  ...
  def fetch(topicPartitionOffsets: Traversable[TopicPartitionValue[Long]]): ReaderT[F, KafkaContext, TopicPartitionValues[OffsetValue[Record]]] = {
  ...
  private def joinGroup(groupId: String, memberId: Option[String], protocol: String, protocols: Seq[GroupProtocol]) = {
    ...
    for {
      groupProtocolRequests <- ReaderT[F, KafkaContext, List[JoinGroupProtocolRequest]](ctx => makeGroupProtocolRequest())
