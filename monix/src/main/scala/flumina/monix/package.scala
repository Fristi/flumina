...
package object monix {
  ...
  implicit class RichKafkaClient(val kafkaClient: KafkaClient) {
    ...
    def produce[R](
      ...
      PartitionConsumer.partitionConsume[R](nrPartitions, buffer, {
        case (nrPartitions, value) => P.selectPartition(value, nrPartitions)
      }) {
        case (part, elems) =>
    ...
    def messages[A](topic: String, consumptionStrategy: ConsumptionStrategy, codecErrorHandler: CodecErrorHandler = CodecErrorHandler.stop)(
