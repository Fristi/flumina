...
private[monix] class TopicConsumer[A](client: KafkaClient, topicPartition: TopicPartition, initialOffset: Long, consumptionStrategy: ConsumptionStrategy, codecErrorHandler: CodecErrorHandler)(
    implicit S: Scheduler,
    D: KafkaDecoder[A])
    extends Observable[TopicPartitionValue[OffsetValue[A]]] {
  ...
  def feedTask(offset: Long, out: Subscriber[TopicPartitionValue[OffsetValue[A]]]): Task[Unit] = {
    Task
      .fromFuture(client.fetch(Set(TopicPartitionValue(topicPartition, offset))))
      .flatMap(values =>
        if (values.errors.nonEmpty) {
          Task.now(out.onError(new TopicErrorException(values.errors)))
        } else {
          def runNext =
            feedTask(maxByOpt(values.success)(_.result.offset).map(_ + 1l).getOrElse(0l), out)
