...
import flumina.core.v090.BasicInterpreter
...
final class AkkaInterpreter(settings: KafkaSettings)(implicit S: ActorSystem, T: Timeout, EC: ExecutionContext) extends (KafkaA ~> Future) {
  ...
  private val connectionPool = S.actorOf(KafkaConnectionPool.props(settings.bootstrapBrokers, settings.connectionsPerBroker))
  ...
  def runControllerCall(dsl: KafkaA[List[TopicResult]]) = {
    ...
    for {
      ...
      res <- if (resp.exists(_.kafkaResult == KafkaResult.NotController)) rerun else Future.successful(resp)
    } yield res
  }
  ...
  override def apply[A](fa: KafkaA[A]): Future[A] = fa match {
    ...
    case r: KafkaA.ProduceOne       => runTopicPartitionValues[Record, Long](Seq(r.values), v => KafkaA.ProduceOne(v.headOption.getOrElse(sys.error("Impossible"))))
